...
try
   Result := true;
     ...
     For i := 1 to 2 do //DIAS DE ADIÇÃO
        ...
        Begin
             Result := False;
             setMensagem('Entre em contato com a Célula Digital Software para obter um serial válido.'+#13+celulaTelefone+#13+'financeiro@celuladigital.com.br','erro');
             PRINCIPAL.Close;
        end;
        ...
        Begin
             Result := False;
             setMensagem('Olá, o SERIAL do sistema está vencido. Entre em contato com a Célula Digital Software para regularizar a situação da empresa.'+#13+celulaTelefone+#13+'financeiro@celuladigital.com.br','informa');
        end;

        if (i <= -4) and (i >= -8) then
        Begin
             Result := False;
             setMensagem('Olá, EVITE CORTES na prestação dos serviços! Entre em contato com a Célula Digital Software para regularizar a situação da empresa!'+#13+celulaTelefone+#13+'financeiro@celuladigital.com.br','erro');
        end;

        if (i < -8) then
        Begin
             Result := False;
             setMensagem('Entre em contato com a Célula Digital Software para regularizar a situação da empresa.'+#13+celulaTelefone+#13+'financeiro@celuladigital.com.br','erro');
             PRINCIPAL.Close;
        end;
...
Begin
     Result := False;
     setMensagem('Entre em contato com a Célula Digital Software para obter um serial válido.'+#13+celulaTelefone+#13+'financeiro@celuladigital.com.br'+#13+#13+#13+e.Message,'erro');
     PRINCIPAL.Close;
end;
end;
end;
...
Begin
     ...
     begin
          ...
          sql.SQL.Text := '';
          sql.SQL.Text := 'SELECT * FROM contracts WHERE num_contrato = '+getConf('EMPRESA_CODIGO_CELULA')+' AND sistema = ''CDS-IMOBILIÁRIA'' AND TRIM(cnpj_cpf) = '''+getConf('EMPRESA_CNPJ')+''' OR TRIM(cnpj_cpf) = '''+getConf('EMPRESA_CNPJ')+''' LIMIT 0 , 1';
          conectaMySQL('celuladigital11','mysql02.celuladigital.com.br','celuladigital11','CDS1005');
          sql.Open;
     ...
     begin
          ...
          unUtilitario.setMensagem('Caro usuário, favor entrar em contato com a Célula Digital para a liberação do sistema.'+#13+'Telefone: '+celulaTelefone,'informa');
          PRINCIPAL.Close;
     end;
...
try
     ...
     begin
          ...
          sql.Connection := PRINCIPAL.MySQL;
          sql.SQL.Text := '';
          sql.SQL.Text := 'SELECT * FROM contracts WHERE sistema = ''CDS-IMOBILIÁRIA'' AND TRIM(cnpj_cpf) = '''+unValidacao.formataCNPJ(cnpj_cpf)+''' OR TRIM(cnpj_cpf) = '''+unValidacao.formataCPF(cnpj_cpf)+''' LIMIT 0 , 1';
          conectaMySQL('celuladigital11','mysql02.celuladigital.com.br','celuladigital11','CDS1005');
          sql.Open;
          ...
          Begin
               unUtilitario.setMensagem('Não existem contratos com este CNPJ ou CPF na Célula Digital.','erro');
               cadastraCNPJ();
          end;
     end
     else
